...
export default function History() {
  const { c } = useTheme()
  const { history, setResult, deleteHistoryItem } = useDetection()
  ...
  const handleDelete = async (e, id) => {
    e.stopPropagation()
    if (!confirm('Are you sure you want to permanently delete this scan and its reports?')) return
    setDeletingId(id)
    try {
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-6">
          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-sm mt-1" style={{ color: c.textDim }}>
                Full chronological database logs of all deepfake scans.
              </p>
            </div>
            <button
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <input
                type="text"
                placeholder="Search by scan ID, filename…"
                value={search}
                      ...
                      {group.items.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex gap-4 items-start">
                            ...
                            {/* Event card */}
                            <div
                              className="flex-1 p-4 rounded-xl transition-all duration-200 cursor-pointer"
                              style={{ background: c.surface, border: `1px solid ${c.border}` }}
                            ...
                            >
                              <div className="flex items-start justify-between gap-3 flex-wrap">
                                ...
                                <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                                  <span className="text-xs font-mono font-bold" style={{ color: iconColor }}>
                                    {(item.confidence * 100).toFixed(1)}%
                                  </span>
                                  <span className="text-xs mr-2" style={{ color: c.textDim }}>{time}</span>
                                  
                                  <button
                                    onClick={(e) => handleDelete(e, item.id)}
                                    disabled={deletingId === item.id}
                                    title="Delete scan"
                                    className="p-1 rounded transition-colors text-zinc-500 hover:text-red-400 hover:bg-red-500/10 flex items-center justify-center disabled:opacity-40"
                                  >
                                    {deletingId === item.id ? (
                                      <div className="w-3.5 h-3.5 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: c.red }} />
                                    ) : (
                              ...
                              <p className="text-xs mt-2 leading-relaxed" style={{ color: c.textDim }}>
                                {item.fileName} • Unified score: {item.fusion?.score?.toFixed(4)} • Risk: {item.riskLevel}
                              </p>
